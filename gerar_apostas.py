...
# Treinando o modelo simples de IA
...
try:
    ...
    if isinstance(dados_api, list):
        for partida in dados_api:
            casa = partida.get("match_hometeam_name")
            fora = partida.get("match_awayteam_name")
            status = partida.get("match_status", "NS")
            
            if casa and fora:
                # Previsão da IA (Métricas estimadas para ordenação)
                diff_pos = -8
                vit_c = 4
                vit_v = 1
...
# Salvar resultados no JSON lido pelo site
with open('dados_apostas.json', 'w', encoding='utf-8') as f:
    json.dump(resultados, f, ensure_ascii=False, indent=2)

print(f"Total de jogos processados para hoje ({hoje}): {len(resultados)}")
